...
# Currently the function is duplicated in two scripts, needs to be 
#  pulled out into a function resource script. 
# TODO
...
cover_reg_tbl <- read_csv(file.path(
  folder_regtbl, 
  "Cover_Disturbance_Tbl.csv"))

height_reg_tbl <- read_csv(file.path(
  folder_regtbl, 
...
file_ch <- list.files(
  ...
  full.names = TRUE)
ch_orig <- rast(file_ch)

# Zones to limit data (i.e. no LF buffer around CONUS)
zones_r <- rast(file.path(folder_mapzones, "LF_zones_rasterized.tif"))
...
cc_reg <- clamp(cc_reg_raw, lower=0, upper=100, values=TRUE)

# Bin it like LF CC 
rcl_cc_bins <- tibble(cc_from = seq(0, 90, by = 10), 
                      cc_to = seq(10, 100, by = 10), 
                      cc_bin = c(0, seq(15, 95, by = 10))) %>% 
  as.matrix()
cc_reg_bin <- classify(cc_reg, rcl = rcl_cc_bins,
                       include.lowest = TRUE,
                       right = FALSE)
...
cc_reg_cg0 <- mask(cc_reg_bin, cg, maskvalues = 0, updatevalue = 0)
# #DEV test
...
cc_reg_cc0 <- mask(cc_reg_cg0, cc_orig, maskvalues = 0, updatevalue = 0)
# #DEV test
...
cc_updtdist <- mask(cc_reg_cc0, dist_bin, maskvalues=1, inverse=TRUE)
...
cc_updt <- cover(cc_updtdist, cc_orig, values = NA)
...
cc_conus <- ifel(!is.na(zones_r), cc_updt, NA)

#Save
writeRaster(cc_conus,
            file.path(
              folder_out_base, 
              version_proj,
...
(start_time_ch <- Sys.time())
ch_reg_raw <- calc_reg_raw("ch")
(end_time_ch <- Sys.time())
(end_time_ch - start_time_ch)

# Min 0 clamp. MUST happen first before binning
ch_min0 <- clamp(ch_reg_raw, lower=0, upper=Inf, values=TRUE)
...
rcl_ch_bins <- tribble(
  ...
  50, Inf, 510) %>% 
  as.matrix()
ch_bin <- classify(ch_min0, rcl = rcl_ch_bins,
                   include.lowest = TRUE,
                   right = FALSE)

# Zero out where canopy guide is 0
ch_cg0 <- mask(ch_bin, cg, maskvalues = 0, updatevalue = 0)

# Zero out where existing CC is 0
ch_cc0 <- mask(ch_cg0, cc_orig, maskvalues = 0, updatevalue = 0)

# Limit to ONLY disturbed pixels
ch_clean <- mask(ch_cc0, dist_bin, maskvalues=1, inverse=TRUE)

## Update existing - CH

# Fill in with baseline values anywhere where there is
#  no updated values (ie. not disturbed & regression calc'd)
ch_updt <- cover(ch_clean, ch_orig, values = NA)
...
# Limit to zones_r, removing LF buffer (matches FBFM40)
ch_conus <- ifel(!is.na(zones_r), ch_updt, NA)


#Save
writeRaster(ch_conus,
            file.path(
              folder_out_base, 
              version_proj,
