...
perims <- read_sf(file.path(
  folder_out_base, version_proj, "disturbance", "fire_perims",
...
pfy25 <- perims %>% 
  mutate(Disc_date = as.Date(Discovery)) %>% 
  filter(Disc_date >= as.Date("2025-10-01")) %>% 
  arrange(Disc_date) %>% 
