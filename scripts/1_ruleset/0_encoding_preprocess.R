...
fvh_csv <- read_csv(file_fvh_csv, show_col_types = FALSE)
fvc_csv <- read_csv(file_fvc_csv, show_col_types = FALSE)
fvt_csv <- read_csv(file_fvt_csv, show_col_types = FALSE)
...
fvc_xwalk <- bind_rows(
  fvc_csv %>%
    ...
    #edit -9999 to have id of NA (will reclass to NA in later step)
    mutate(fvc_ufid = NA) %>% 
    dplyr::select(fvc_ufid, everything()), 
  fvc_csv %>% 
    filter(!VALUE == -9999) %>% 
    mutate(fvc_ufid = row_number()) %>% 
    dplyr::select(fvc_ufid, everything()))
...
fvt_xwalk <- bind_rows(
  fvt_csv %>%
    ...
    #edit -9999 to have id of NA (will reclass to NA in later step)
    mutate(fvt_ufid = NA) %>% 
    dplyr::select(fvt_ufid, everything()), 
  fvt_csv %>% 
    filter(!VALUE == -9999) %>% 
    mutate(fvt_ufid = row_number()) %>% 
    dplyr::select(fvt_ufid, everything()))


# Save out 
write_csv(fvh_xwalk, 
          file.path(folder_xwalk, 
