...
fvh <- terra::rast(
  file.path(
    ...
    "LF2024_FVH_CONUS",
    "Tif",
    "LF2024_FVH_CONUS.tif"
  )
)

folder_out_0km <- file.path(folder_mapzones, "per_zone_rasters_0km")
dir.create(folder_out_0km)

folder_out_90km <- file.path(folder_mapzones, "per_zone_rasters_90km")
dir.create(folder_out_90km)

### Zones rasterized -----------------------------------------------------------

# 0 km version
zones0km_r <- terra::rasterize(
  zones0km_sf,
  fvh,
  field = "ZONE_NUM",
  background = NA,
...
for (i in seq_along(zones)) {
  ...
  #7 min
  this_zone0km_r <- terra::mask(
    zones0km_r,
    zones0km_r,
    this_zone_num,
    inverse = TRUE
  )
  this_zone0km_trim <- terra::trim(this_zone0km_r, padding = 0, value = NA)

  #saving
  terra::writeRaster(
    this_zone0km_trim,
    file.path(folder_out_0km, paste0("z", this_zone_pad, ".tif")),
    gdal = c(compress = "DEFLATE"),
    overwrite = TRUE
  )
  ...
  this_zone90km_trim <- terra::trim(this_zone90km_r, padding = 0, value = NA)

  #saving
  terra::writeRaster(
    this_zone90km_trim,
    file.path(folder_out_90km, paste0("z", this_zone_pad, ".tif")),
    gdal = c(compress = "DEFLATE"),
    overwrite = TRUE
  )
}
