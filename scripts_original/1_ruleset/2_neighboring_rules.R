...
fvh_xwalk <- readr::read_csv(file_fvh_xwalk, show_col_types = FALSE) %>%
  dplyr::select(VALUE, fvh_ufid)

fvc_xwalk <- readr::read_csv(file_fvc_xwalk, show_col_types = FALSE) %>%
  dplyr::select(VALUE, fvc_ufid)

fvt_xwalk <- readr::read_csv(file_fvt_xwalk, show_col_types = FALSE) %>%
...
#centroids
zone_c <- sf::st_centroid(zones_sf)
...
zone_c_idx <- zone_c %>%
  ...
  dplyr::select(idx, tidyselect::everything())
...
for (i in 1:nrow(zone_c_idx)) {
  ...
  this_cmb <- readr::read_csv(
    file.path(
      folder_cmb,
      paste0("LF", version_target, "_z", this_zone_pad, "_CMB.csv")
    ),
  ...
  #double encoding
  this_cmb <- this_cmb %>%
    dplyr::left_join(fvh_xwalk, by = dplyr::join_by("EVHR" == "VALUE")) %>%
    #filter(is.na(fvh_ufid))
    dplyr::left_join(fvc_xwalk, by = dplyr::join_by("EVCR" == "VALUE")) %>%
    #filter(is.na(fvc_ufid))
    dplyr::left_join(fvt_xwalk, by = dplyr::join_by("EVTR" == "VALUE")) %>%
    #filter(is.na(fvt_ufid))
    dplyr::mutate(bps_ufid = dplyr::if_else(BPSRF == -1111, 8888, BPSRF)) %>%
    #large values
    dplyr::mutate(
      encoded = DIST *
        1e11 +
  ...
  for (j in seq_along(this_neighbors)) {
    ...
    this_n_zone_num <- zone_c_idx %>%
      dplyr::filter(idx == this_n_index) %>%
      dplyr::pull(ZONE_NUM)

    this_n_zone_pad <- this_n_zone_num %>%
      stringr::str_pad(2, pad = "0")

    this_n_cmb <- readr::read_csv(
      file.path(
        folder_cmb,
        paste0("LF", version_target, "_z", this_n_zone_pad, "_CMB.csv")
      ),
      show_col_types = FALSE
    ) %>%
      dplyr::select(-tidyselect::any_of(drop_cols)) %>%
