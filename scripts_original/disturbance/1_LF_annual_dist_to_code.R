...
fname_base <- paste0(
  ...
  stringr::str_sub(dist_year, start = -2, end = -1)
)
...
dist_csv <- readr::read_csv(file_dist_csv)
...
dist_r <- terra::rast(file_dist_rast)
...
dist_dist <- dist_csv %>%
  ...
  dplyr::mutate(diff_years = .env$yr_diff) %>%
  #join all xwalks, dropping those unmatched (don't want NAs)
  dplyr::inner_join(dtype_xwalk, by = dplyr::join_by(DIST_TYPE)) %>%
  dplyr::inner_join(dsev_xwalk, by = dplyr::join_by(SEVERITY)) %>%
  dplyr::inner_join(dtime_xwalk, by = join_by(diff_years)) %>%
  # make DIST code
  dplyr::mutate(DIST = paste0(D_TYPE, D_SEVERITY, D_TIME) %>% as.numeric())

# Get NoData (-9999), NA (0), and Water mask value (sometimes 2001)
dist_spec <- dist_csv %>%
  dplyr::filter(DIST_TYPE %in% c("Fill-NoData")) %>%
  dplyr::mutate(DIST = NA_real_) %>%
  dplyr::rows_append(
    dist_csv %>%
      dplyr::filter(DIST_TYPE %in% c(NA, "Water")) %>%
      dplyr::mutate(DIST = 0)
  ) %>%
  dplyr::select(VALUE, DIST_TYPE, SEVERITY, DIST)

# Full xwalk for reclassifying (and checking)
dist_codes <- dplyr::bind_rows(dist_spec, dist_dist)
...
terra::activeCat(dist_r) <- 0
dist_r
dist_r_coded <- terra::classify(
  dist_r,
  rcl = dist_rcl,
...
terra::writeRaster(
  dist_r_coded,
  file.path(folder_out, paste0(dist_vers, "_", dist_year, "_dist_coded.tif")),
