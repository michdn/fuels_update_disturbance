...
fvh_rcl <- readr::read_csv(file_fvh_xwalk, show_col_types = FALSE) %>%
  dplyr::select(VALUE, fvh_ufid) %>%
  dplyr::rename(IS = VALUE, BECOMES = fvh_ufid) %>%
  as.matrix()

fvc_rcl <- readr::read_csv(file_fvc_xwalk, show_col_types = FALSE) %>%
  dplyr::select(VALUE, fvc_ufid) %>%
  dplyr::rename(IS = VALUE, BECOMES = fvc_ufid) %>%
  as.matrix()

fvt_rcl <- readr::read_csv(file_fvt_xwalk, show_col_types = FALSE) %>%
  dplyr::select(VALUE, fvt_ufid) %>%
  dplyr::rename(IS = VALUE, BECOMES = fvt_ufid) %>%
  as.matrix()

# Reclass / FV* encoding (first of the double encoding)
#others = -1 to see if any did not reclass correctly
fvh <- terra::classify(fvh_orig, rcl = fvh_rcl, others = -1)
fvc <- terra::classify(fvc_orig, rcl = fvc_rcl, others = -1)
fvt <- terra::classify(fvt_orig, rcl = fvt_rcl, others = -1)
...
fvh_min <- terra::global(fvh, min, na.rm = TRUE)
fvc_min <- terra::global(fvc, min, na.rm = TRUE)
fvt_min <- terra::global(fvt, min, na.rm = TRUE)
# per 1. global() 2 min; min() 7 min; # app() almost 10 min
if (fvh_min == -1) {
  stop("FVH classification failed some values.")
}
if (fvc_min == -1) {
  stop("FVC classification failed some values.")
}
if (fvt_min == -1) {
  stop("FVT classification failed some values.")
}

## Save out
terra::writeRaster(
  fvc,
  file.path(folder_lfproc, paste0("LF", version_target, "_fvc_coded.tif")),
  gdal = c("COMPRESS=DEFLATE")
)
terra::writeRaster(
  fvh,
  file.path(folder_lfproc, paste0("LF", version_target, "_fvh_coded.tif")),
  gdal = c("COMPRESS=DEFLATE")
)
terra::writeRaster(
  fvt,
  file.path(folder_lfproc, paste0("LF", version_target, "_fvt_coded.tif")),
  gdal = c("COMPRESS=DEFLATE")
)
...
fbfm <- terra::mask(fbfm, fbfm, maskvalues = c(-1111, -9999), updatevalue = NA)

terra::writeRaster(
  fbfm,
  file.path(folder_lfproc, paste0("LF", version_target, "_fbfm.tif")),
  gdal = c("COMPRESS=DEFLATE")
)
...
process_canopy <- function(c_var) {
  this_file <- list.files(
    path = folder_lfdata_base,
    pattern = paste0("^LF", version_target, "_", c_var, "_CONUS\\.tif$"),
    recursive = TRUE,
    full.names = TRUE
  )
  ...
  terra::writeRaster(
    ...
    file.path(
      folder_lfproc,
      paste0("LF", version_target, "_", tolower(c_var), ".tif")
    ),
    gdal = c("COMPRESS=DEFLATE")
  )
}
...
terra::writeRaster(
  bps_ext,
  file.path(folder_lfproc, paste0("LF", version_bps, "_bps_coded.tif")),
  gdal = c("COMPRESS=DEFLATE")
)
