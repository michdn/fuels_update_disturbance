...
fvc_lut <- readr::read_csv(file.path(
  folder_lut,
  "LF2020_LUT_Cover.csv"
))

fvh_lut <- readr::read_csv(file.path(
  folder_lut,
  "LF2020_LUT_Height.csv"
))
...
fvc_mid <- terra::classify(fvc_orig, rcl = fvc_rcl, others = NA)
names(fvc_mid) <- "FVC_MIDPOINT"
varnames(fvc_mid) <- "FVC_MIDPOINT"

fvh_mid <- terra::classify(fvh_orig, rcl = fvh_rcl, others = NA)
...
terra::writeRaster(
  fvc_mid,
  file.path(folder_lfproc, paste0("LF", version_target, "_fvc_midpoints.tif")),
  gdal = c("COMPRESS=DEFLATE")
)
terra::writeRaster(
  fvh_mid,
  file.path(folder_lfproc, paste0("LF", version_target, "_fvh_midpoints.tif")),
  gdal = c("COMPRESS=DEFLATE")
)
